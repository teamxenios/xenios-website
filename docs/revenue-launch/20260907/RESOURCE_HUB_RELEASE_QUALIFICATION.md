...
A subsequent bounded check of the two founder-named Resource Hub/code-repair packages found the Resource Hub package's `SOURCE_REGISTER.md` (SHA-256 `919666431b0858184597c776ca4395dd934e0e2d298280c7edb6adbb199a37a0`). Its references do not identify the required internal team, advisor or counterparty documents. Six checked package records and their hashes are recorded in the [V3 source gate](privacy-input-v3-source-gate.json). No similarly named document elsewhere was selected, no unrelated account was searched and no further name extraction occurred. The required input remains the exact authorized document paths or the register that identifies them.
