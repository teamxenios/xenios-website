...
2. Inspect only the relevant configuration. For installation explicitly set the service-level `RESEARCH_RESOURCE_HUB_ENABLED=false` and verify that stored value before any candidate deployment. Keep membership billing absent/false; do not activate commerce or other workstreams. Existing Supabase credentials must stay secret. Stored flag state is not proof of an already-running process's state.
...
8. Run the same 30 public/read-only endpoint capture and existing comparison with zero unexplained regressions or human-review-required differences. Verify public health and unauthenticated account/admin boundaries. No real account/partner approval, notification job, email, payment, shipment, resource upload/publication or download audit is part of installation smoke.
