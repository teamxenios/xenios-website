...
- **Approval-to-login browser journey: PASS in bounded nonproduction qualification.** The dedicated harness used isolated admin/customer Chromium profiles and the real inspection, approval, claim, token-validation, Auth session, account, sign-out, and protected-route UI paths over local synthetic boundaries. Ordered assertions passed: fresh absence, actual admin UI approval, generated local claim notification, signed-out claim/password creation, normal login, active account with no membership billing, refresh, rendered sign-out, denial while signed out, sign-in again, and no automatic partner/admin role. Evidence: committed runner `scripts/revenue-launch/new-account-browser-qualification.mjs` plus `new-account-browser-qualification-20260906.json` (SHA-256 `eac234d815ead9464aa23e7a1787a757ea70a382356800de1a84a64c7b2bf2ab`). Existing-account and partner-isolation evidence remains separately recorded in `account-journey-browser-20260906.json`. No live account approval, claim, email, or notification was performed.
...
The account/partner qualification prerequisites are now **PASS** for the selected candidate. A defensible deployment GO still requires the founder's exact-SHA production authorization naming migrations, rollback actions, and post-deployment smoke checks. Resource Hub work remains separate and does not block this narrow release.
