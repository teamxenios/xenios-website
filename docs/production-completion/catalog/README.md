...
- Identity matching is exact normalization only. No fuzzy, semantic, supplier,
  price, or demand-based match is accepted. The only duplicate-label resolution
  uses an exact form/subcategory equality check.
...
- `assisted-order-batch.json` — exact canonical/bound RUO rows with positive
  candidate retail and landed-cost evidence at or above the sourced margin floor.
...
- `unavailable-batch.json` — unmatched, absent-current-intake, approval-gated,
  unbound, or economically blocked rows.
