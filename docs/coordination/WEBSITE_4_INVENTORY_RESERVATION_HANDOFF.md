...
- Production build: `npm run build` passed with only pre-existing chunk/import
  warnings.
- Diff check: passed.
- Fresh disposable PostgreSQL 16 bootstrap: passed.
- Candidate migration apply twice: passed.
- Canonical raw Git-blob migration identity: 49,917 bytes, SHA-256
...
2. Compose the production implementation behind future authenticated
   server-authoritative callers.
3. Keep checkout disabled until the caller supplies authenticated actor/member
   identity, a reviewed hold policy, stable command idempotency, and uses this
   port instead of the dormant direct-DML store.
4. Do not expose lot allocations outside future server-side order persistence.
5. Apply through the normal production migration path, then rerun the
