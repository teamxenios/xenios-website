...
| POST | `/api/operations/mitch/orders/:orderId/exceptions/:exceptionId/resolve` | same | Mitch/logistics | staff assignment | atomic fulfillment RPC + exception/audit/outbox evidence | route + SQL behavior tests | enable after migration/wiring |
...
| GET | `/api/admin/research/operations/inventory/lots` | same | verified admin | admin scope | canonical inventory lots | route tests | enable after migration/wiring |
| POST | `/api/admin/research/operations/inventory/lots/:lotId/commands` | same | verified admin | admin scope + version/idempotency | atomic inventory RPC + append-only command/movement evidence | route + SQL behavior tests | enable after migration/wiring |
...
| GET | `/api/admin/research/operations/crm/:contactId` | same | verified admin | admin scope | CRM contact + append-only timeline | route + SQL behavior tests | enable after migration 31/wiring |
| POST | `/api/admin/research/operations/crm` | same | verified admin | admin scope | atomic CRM RPC | route + SQL behavior tests | enable after migration 31/wiring |
| POST | `/api/admin/research/operations/crm/:contactId/stage` | same | verified admin | admin scope | atomic CRM RPC + optimistic concurrency | route + SQL behavior tests | enable after migration 31/wiring |
| POST | `/api/admin/research/operations/crm/:contactId/notes` | same | verified admin | admin scope | privacy-filtered atomic CRM RPC | route + SQL behavior tests | enable after migration 31/wiring |
| POST | `/api/admin/research/operations/crm/:contactId/references` | same | verified admin | admin scope | operational-reference CRM event | route + SQL behavior tests | enable after migration 31/wiring |
