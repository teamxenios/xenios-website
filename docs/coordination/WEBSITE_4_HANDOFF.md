...
17. **Idempotency/audit:** Fulfillment, partner-request, and professional-account RPCs are idempotent, stale-write protected where applicable, transition checked, and audited. Append-only evidence cannot be updated or deleted.
18. **Migration verification:** The migration was applied twice to a disposable PostgreSQL 16 database after migrations 1–26. Fulfillment, partner-request replay, the full professional pipeline, RLS, grants, and record-count invariants passed.
...
26. **Newly finished missing scope:** Durable partner campaign/event/organization/compliance intake, aggregate partner reports, hashed partner session history, and the complete prospect → discovery → diligence → commercial review → agreement → active/paused/closed professional pipeline.
