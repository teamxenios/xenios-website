...
18. **Migration verification:** Migration 31 was applied twice to a disposable PostgreSQL 16 database after migrations 1–26. Fulfillment, assigned-task replay/stale-write handling, partner-request replay, the full professional pipeline, RLS, grants, and record-count invariants passed.
...
26. **Newly finished missing scope:** Durable assigned operations tasks, partner campaign/event/organization/compliance intake, aggregate partner reports, hashed partner session history, and the complete prospect → discovery → diligence → commercial review → agreement → active/paused/closed professional pipeline.
