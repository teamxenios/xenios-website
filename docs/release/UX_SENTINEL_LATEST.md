...
| SEN-0013 | P2, P1 for any fresh approved member | The activation email's Continue activation link lands on the review password gate: /research/activate is not in the RecoveryChrome bypass (only sign-in and reset-password are), so an approved member with a fresh browser cannot claim their account. One production member is pending activation. |
