...
Founder-issued 2026-08-21. Paste this into any Claude/Fable/Codex worker session
to put it into emergency worker mode: recover real state, claim ONE unowned lane
(A–I below), code immediately, push exact SHAs to the main lead, then take the
next unowned lane instead of going idle. The MAIN Claude session is the only
integrator / release owner / production owner / RC owner. Companion:
`UNIVERSAL_AGNOSTIC_WORKER_JOIN_PROMPT.md`.
