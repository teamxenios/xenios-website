...
describe("supplement media rights policy", () => {
  ...
  it.each([
    "Internal use only. Publishing prohibited.",
    "Publishing prohibited.",
    "Unmodified use on reseller pages only",
  ])("fails closed on non-allowlisted rights text: %s", (limitations) => {
    const result = validateRightsEvidence({ ...approvedRights, limitations }, at);
    expect(result.valid).toBe(false);
    expect(result.reasons).toContain(
      "rights limitations are not explicitly approved for public use",
    );
  });
  ...
  ])("accepts only absent or explicitly allowlisted publication limitations: %s", (limitations) => {
    expect(validateRightsEvidence({ ...approvedRights, limitations }, at)).toEqual({
      valid: true,
      reasons: [],
    });
  });
