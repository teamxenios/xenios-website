...
const PUBLICATION_DENIAL = /\b(?:internal(?:\s+review)?\s+only|do\s+not\s+(?:use|publish|republish)|not\s+for\s+(?:public\s+use|publication|republication)|no\s+(?:public\s+use|publication|republication)|(?:publication|republication|republishing)\s+(?:is\s+)?(?:not\s+permitted|prohibited))\b/i;
...
export function validateRightsEvidence(
  ...
  if (evidence.limitations && PUBLICATION_DENIAL.test(evidence.limitations)) {
    reasons.push("rights limitations prohibit public use");
  }
