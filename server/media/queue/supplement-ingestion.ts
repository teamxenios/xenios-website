...
import { deriveApprovalState } from "../rights/policy";
...
export async function runSupplementIngestionBatch(input: {
  ...
  for (const row of input.rows) {
    ...
    if (previousJob && previousMedia && ["AWAITING_REVIEW", "APPROVED", "LINKED", "HELD"].includes(previousJob.status)) {
      jobs.push(previousJob);
      media.push(previousMedia);
      continue;
    }
    ...
    try {
      ...
      record.approvalStatus = deriveApprovalState(match.state, rights.status);
  ...
  return {
    ...
    summary: {
      ...
      rightsApproved: media.filter((item) => approvedRights.has(item.rights.status)).length,
