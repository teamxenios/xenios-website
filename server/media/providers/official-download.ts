...
import type { SupplementBrand, SupplementMediaRecord } from "../official-sources/contracts";
import { assertOfficialUrl, type FetchLike } from "../official-sources/http";
...
export function assertPublicImageUrl(brand: SupplementBrand, value: string): URL {
  const parsed = new URL(value);
  const hostname = parsed.hostname.toLowerCase().replace(/^\[|\]$/g, "").replace(/\.$/, "");
  if (parsed.protocol !== "https:") throw new Error("Image URL must use HTTPS");
  if (parsed.username || parsed.password) throw new Error("Image URL must not contain credentials");
  ...
  const url = assertOfficialUrl(brand, value);
  url.hash = "";
  return url;
}
...
export async function downloadApprovedOfficialOriginal(input: {
  ...
  let url = assertPublicImageUrl(input.record.brand, input.record.sourceImageUrl);
  ...
  for (let redirects = 0; redirects <= 2; redirects += 1) {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      url = assertPublicImageUrl(input.record.brand, new URL(location, url).toString());
      continue;
    }
    break;
  }
