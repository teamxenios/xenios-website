...
describe("Care clinical write chokepoint", () => {
  ...
  it("refuses a clinical read while real patient data is disabled, and reads nothing", async () => {
    ...
    const appointmentRead = await request(h.app).get("/api/care/appointments");
    ...
    expect(appointmentRead.status).toBe(403);
    expect(appointmentRead.body.capability).toBe("real_patient_data");
    ...
    expect(h.appointments.listPatientAppointments).not.toHaveBeenCalled();
  });
  ...
  it("refuses every clinical write aimed directly at the API, and writes nothing", async () => {
    ...
    const attempts = [
      ...
      request(h.app)
        ...
        .send({ intakeId: INTAKE, idempotencyKey: "direct-key-appt-01" }),
      request(h.app)
        .post(`/api/care/appointments/${APPOINTMENT}/action`)
        .send({ action: "check_in", expectedVersion: 0, idempotencyKey: "direct-key-appt-02" }),
      request(h.app)
        .post(`/api/care/appointments/${APPOINTMENT}/assign`)
        .send({ clinicianUserId: ADMIN, idempotencyKey: "direct-key-appt-03" }),
      request(h.app)
        ...
        .send({
          ...
          startsAt: NOW,
          endsAt: NOW,
          idempotencyKey: "direct-key-appt-04",
        }),
      request(h.app)
        .post(`/api/care/appointments/${APPOINTMENT}/no-show`)
        .send({ expectedVersion: 0, idempotencyKey: "direct-key-appt-05" }),
      request(h.app)
    ...
    expect(h.appointments.requestAppointment).not.toHaveBeenCalled();
    expect(h.appointments.patientAction).not.toHaveBeenCalled();
    expect(h.appointments.assignClinician).not.toHaveBeenCalled();
    expect(h.appointments.scheduleAppointment).not.toHaveBeenCalled();
    expect(h.appointments.adminMarkNoShow).not.toHaveBeenCalled();
    ...
    expect(refusals).toEqual([
      {
        operation: "prescription.sign",
        reviewAction: null,
        requiredCapabilities: ["prescribing"],
        missingCapabilities: ["prescribing"],
  ...
  it("requires both scheduling capabilities on the background chokepoint", async () => {
    const work = vi.fn(async () => "scheduled");
    const attempts = [
      {
        flags: { ...ALL_OFF, provider_actions: true },
        missing: ["external_communications"],
      },
      {
        flags: { ...ALL_OFF, external_communications: true },
        missing: ["provider_actions"],
      },
    ] as const;
    ...
    for (const attempt of attempts) {
      ...
      await expect(
        runCareClinicalWrite(
          ...
          work,
          {
            readFlags: () => attempt.flags,
            recordRefusal: (event) => refusals.push(event),
            now: () => new Date(NOW),
          },
        ),
      ...
      expect(refusals[0]).toMatchObject({
        ...
        missingCapabilities: attempt.missing,
        surface: "background",
      });
    }
  ...
  it("records an audit event on refusal that carries no patient or clinical content", async () => {
    ...
    expect(h.refusals).toEqual([
      {
        ...
        requiredCapabilities: ["external_communications"],
        missingCapabilities: ["external_communications"],
    ...
    expect(serialized).not.toContain("intakeId");
  });
...
describe("Care clinical write chokepoint, failing closed", () => {
  ...
  it("requires every capability in the scheduling set, not merely either one", () => {
    const providerOnly = evaluateCareClinicalWrite(
      { operation: "appointment.schedule" },
      { ...ALL_OFF, provider_actions: true },
    );
    expect(providerOnly.allowed).toBe(false);
    expect(providerOnly.requiredCapabilities).toEqual([
      "provider_actions",
      "external_communications",
    ]);
    expect(providerOnly.missingCapabilities).toEqual([
      "external_communications",
    ]);

    const communicationsOnly = evaluateCareClinicalWrite(
      { operation: "appointment.schedule" },
      { ...ALL_OFF, external_communications: true },
    );
    expect(communicationsOnly.allowed).toBe(false);
    expect(communicationsOnly.missingCapabilities).toEqual([
      "provider_actions",
    ]);

    const both = evaluateCareClinicalWrite(
      { operation: "appointment.schedule" },
      {
        ...ALL_OFF,
        provider_actions: true,
    ...
    expect(both.missingCapabilities).toEqual([]);
  });
  ...
  it("maps every static operation to a nonempty set of the five named capabilities", () => {
...
describe("Care nonclinical eligibility routes stay usable with every clinical capability off", () => {
  it("keeps the eligibility location write outside the clinical gate", async () => {
    const h = harness(ALL_OFF);
