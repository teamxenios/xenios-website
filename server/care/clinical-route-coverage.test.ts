...
import {
  ...
  careClinicalOperationCapabilities,
 ...
 * mounted for. Appointment scheduling is still one canonical gate even though
 * its decision requires two independent capabilities.
 */
const CLINICAL_ROUTES: Readonly<Record<string, CareClinicalOperation>> = {
  ...
  "GET /api/care/appointments": "appointment.read_self",
  "POST /api/care/appointments": "appointment.request",
 ...
 * so a route added anywhere in the registered Care modules shows up
 * automatically.
 */
...
describe("Care route coverage: every clinical route uses the one centralized gate", () => {
  ...
  it("maps every clinical route to a nonempty canonical capability set", () => {
    for (const operation of Object.values(CLINICAL_ROUTES)) {
      if (operation === "review.action") {
        ...
        expect(careClinicalOperationCapabilities(operation)).toBeNull();
        continue;
      }
      const capabilities = careClinicalOperationCapabilities(operation);
      expect(capabilities).not.toBeNull();
      expect(capabilities!.length).toBeGreaterThan(0);
    }
    expect(careClinicalOperationCapabilities("appointment.schedule")).toEqual([
      "provider_actions",
  ...
  it("keeps the sixteen clinical writes and four clinical reads exactly", () => {
  ...
  it("pins the exact fifteen appointment, prescription, and pharmacy routes plus intake and review", () => {
    const clinical = registered.filter((route) => route.gateOperation !== null);
    const appointmentPrescriptionPharmacy = clinical
      .filter((route) =>
        route.path.startsWith("/api/care/appointments") ||
        route.path.startsWith("/api/care/prescriptions") ||
        route.path.startsWith("/api/care/pharmacy"),
      )
      .map((route) => `${route.key} -> ${route.gateOperation}`)
      .sort();

    expect(appointmentPrescriptionPharmacy).toEqual([
      "GET /api/care/appointments -> appointment.read_self",
      "GET /api/care/pharmacy/orders -> pharmacy.read_orders",
      "GET /api/care/prescriptions -> prescription.read_self",
      "POST /api/care/appointments -> appointment.request",
      "POST /api/care/appointments/:appointmentId/action -> appointment.patient_action",
      "POST /api/care/appointments/:appointmentId/assign -> appointment.assign_clinician",
      "POST /api/care/appointments/:appointmentId/complete -> appointment.clinician_complete",
      "POST /api/care/appointments/:appointmentId/no-show -> appointment.no_show",
      "POST /api/care/appointments/:appointmentId/schedule -> appointment.schedule",
      "POST /api/care/pharmacy/admin/orders/:orderId/clarification/resolve -> pharmacy.resolve_clarification",
      "POST /api/care/pharmacy/admin/prescriptions/:prescriptionId/assign -> prescription.assign_pharmacy",
      "POST /api/care/pharmacy/orders/:orderId/action -> pharmacy.order_action",
      "POST /api/care/prescriptions -> prescription.create_draft",
      "POST /api/care/prescriptions/:prescriptionId/sign -> prescription.sign",
      "POST /api/care/prescriptions/pharmacy-orders/:orderId/clarification/resolve -> pharmacy.resolve_clarification",
    ]);
    ...
    expect(
      clinical
        .filter(
          (route) =>
            route.path.startsWith("/api/care/intake") ||
            route.path === "/api/care/reviews/:reviewId/action",
        )
        .map((route) => `${route.key} -> ${route.gateOperation}`)
        .sort(),
    ).toEqual([
...
describe("Care route coverage: patient appointment writes have no outbound dispatcher", () => {
  const routeSource = readFileSync(
    resolve(__dirname, "appointment-routes.ts"),
    "utf8",
  );
  const repositorySource = readFileSync(
    resolve(__dirname, "appointment-repository.ts"),
    "utf8",
  );
  const migrationSource = readFileSync(
    resolve(
      ...
      "..",
      "supabase",
      "care-appointments-clinician.sql",
    ),
    "utf8",
  );
  ...
  function sqlFunction(name: string): string {
    ...
    expect(start, `${name} is absent from the canonical migration`).toBeGreaterThanOrEqual(0);
  ...
  it("keeps request, cancel, and check-in on local RPC persistence only", () => {
    // Source evidence for the capability decision: the HTTP handlers have no
    // outbound adapter, their repository methods are direct RPC wrappers, and
    // both canonical SQL functions only mutate Care tables. If a notifier,
    // webhook, or HTTP dispatch is introduced, this proof fails and these
    // operations must also require external_communications.
    expect(routeSource).not.toMatch(
      /(?:notify|notification|webhook|dispatch|fetch\s*\(|axios|http_post|net\.http)/i,
    );
    expect(repositorySource).toMatch(
      /requestAppointment:\s*\(input\)\s*=>\s*rpcAppointment\("care_request_appointment"/,
    );
    expect(repositorySource).toMatch(
      /patientAction:\s*\(input\)\s*=>\s*rpcAppointment\("care_patient_appointment_action"/,
    );

    const outboundMechanism =
      /(?:notify|notification|webhook|dispatch|http_post|net\.http|pg_net|outbox)/i;
    expect(sqlFunction("care_request_appointment")).not.toMatch(
      outboundMechanism,
    );
    expect(sqlFunction("care_patient_appointment_action")).not.toMatch(
      outboundMechanism,
    );
    expect(careClinicalOperationCapabilities("appointment.request")).toEqual([
      "real_patient_data",
    ]);
    expect(
      careClinicalOperationCapabilities("appointment.patient_action"),
    ).toEqual(["real_patient_data"]);
  });
});
 ...
 * three live routes (including the public Tebra configuration), so an ungated
 * clinical route added there was invisible to
...
const SENTINEL = {
  ...
  appointmentId: "sentinel-appointment-id-9c41",
  intakeId: "sentinel-intake-id-9c41",
...
const SENTINEL_APPOINTMENT = {
  id: SENTINEL.appointmentId,
  patientId: SENTINEL.patientId,
  intakeId: SENTINEL.intakeId,
...
function proofApp(serviceCoverageActive = true) {
  ...
  registerCareApi(app, deps, {
    ...
    env: {
      ...
      TEBRA_ALLOWED_ORIGINS: "https://provider.example",
      TEBRA_ENVIRONMENT: "review",
      TEBRA_SCHEDULING_URL:
        `https://provider.example/schedule?patientId=${SENTINEL.patientId}&intakeId=${SENTINEL.intakeId}`,
      TEBRA_PATIENT_PORTAL_URL:
        `https://provider.example/portal?appointmentId=${SENTINEL.appointmentId}`,
    },
  });
...
const RESPONSE_PROOFS: Readonly<Record<string, ResponseProof>> = {
  ...
  "GET /api/care/tebra/configuration": {
    method: "get",
    path: "/api/care/tebra/configuration",
    forbid: ALL_SENTINELS,
  },
  "GET /api/care/eligibility": {
    method: "get",
