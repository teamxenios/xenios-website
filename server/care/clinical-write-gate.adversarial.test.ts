...
/** The eleven clinical WRITE routes. */
...
/** The three clinical READS. Each returns a real person's clinical content. */
...
describe("adversarial: every clinical write refuses with all five flags unset", () => {
  ...
  it("refuses all eleven clinical writes in one pass and writes zero rows across every repository", async () => {
    const { app, repositories } = productionShapedApp();
...
describe("adversarial: a failing audit sink cannot convert a refusal into an allow", () => {
  ...
  it("records the refusal through the default sink with no clinical content in the line", async () => {
    ...
    for (const secret of [PATIENT, REVIEW, "synthetic", draftPayload.idempotencyKey]) {
      expect(warned[0]).not.toContain(secret);
    }
  });
});

describe("adversarial: the legitimate nonclinical referral path stays functional", () => {
  const appointment = {
    id: APPOINTMENT,
    patientId: PATIENT,
    intakeId: INTAKE,
  ...
  it("carries a tebra referral through untouched with all five flags off", async () => {
  ...
  it("keeps the other four nonclinical appointment writes working with all five flags off", async () => {
    const { app, appointments } = workingApp();
    ...
    expect(appointments.patientAction).toHaveBeenCalledTimes(1);
    expect(appointments.assignClinician).toHaveBeenCalledTimes(1);
    expect(appointments.adminMarkNoShow).toHaveBeenCalledTimes(1);
  });
