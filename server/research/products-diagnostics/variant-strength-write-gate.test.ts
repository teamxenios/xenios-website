...
describe("the two exploits an adversarial review used to defeat the price gate", () => {
  ...
  it("EXPLOIT A: ProductAdminService cannot walk a priced clean variant ONTO a disputed SKU", async () => {
    const dispute = DISPUTED[0];
    ...
    const { repo, service: admin } = service(record);
    await admin.createPrice(
      "product-1",
      priceInput,
      "operator@verification.invalid",
      "exploit-a-price",
    );
    await admin.approvePrice(
      "product-1",
      "price-1",
      "operator@verification.invalid",
      "exploit-a-approve",
  ...
  it("EXPLOIT B: ProductAdminService cannot rename a disputed variant to escape the guard", async () => {
    const dispute = DISPUTED[0];
    const record = detail({
    ...
    const refusal = await refusalFrom(
      admin.updateVariant(
        ...
        "variant-1",
        {
          ...
          catalogNumber: "RENAMED-CATALOG-NUMBER",
        },
        "operator@verification.invalid",
        "exploit-b",
      ),
    ...
    expect(refusal.code).toBe("variant_strength_disputed");
    expect(refusal.reason).toContain("renaming the unit is not resolving the dispute");
    expect(repo.updateVariant).not.toHaveBeenCalled();
  });
  ...
  it("ProductAdminService permits a lifecycle-only edit and performs exactly one mutation", async () => {
  ...
  it("fails closed through ProductAdminService when the current SKU is blank", async () => {
    const dispute = DISPUTED[0];
    const record = detail({
      variants: [
        variant({
          id: "variant-1",
          sku: "",
          catalogNumber: dispute.sku,
          strength: dispute.founderLocked.presentation,
        }),
      ],
    });
    const direct = screenVariantEdit(record, "variant-1", { sku: "CLEAN-LOOKING-SKU" });
    expect(direct?.code).toBe("variant_identity_unresolved");
    const { repo, service: admin } = service(record);
    const refusal = await refusalFrom(
      admin.updateVariant(
        "product-1",
        "variant-1",
        { sku: "CLEAN-LOOKING-SKU" },
        "operator@verification.invalid",
        "blank-current-sku",
      ),
    );
