...
function fixture(options: { pages?: unknown[][]; authError?: boolean; failedTable?: string; records?: Record<string, unknown[]> } = {}) {
  const reads: Read[] = [];
  ...
  const db = { auth: { admin: { listUsers } }, from } as unknown as SupabaseClient;
  return { reads, from, listUsers, deps: createAccessInspectionDependencies(() => db, {}) };
}
