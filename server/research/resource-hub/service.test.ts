...
describe("RH-B28-1b: a decoy stream token cannot hide a real object stream", () => {
  const { deflateSync } = require("node:zlib") as typeof import("node:zlib");
  const L = (s: string) => Buffer.from(s, "latin1");
  const judge = (bytes: Buffer) => validatePdfUpload({ bytes, declaredContentType: "application/pdf", originalFilename: "decoy.pdf" });
  const ACTION = "1 0 << /Type /Action /S /JavaScript /JS (app.alert(1)) >>";
  function objStm(prefix: string): Buffer {
    const body = deflateSync(L(ACTION));
    return Buffer.concat([
      L(`%PDF-1.5\n${prefix}2 0 obj << /Type /ObjStm /N 1 /First 0 /Filter /FlateDecode /Length ${body.byteLength} >>\nstream\n`),
      body,
      L("\nendstream\nendobj\n%%EOF\n"),
    ]);
  }

  it("baseline: the same object stream without a decoy is refused", () => {
    expect(judge(objStm("")).reasons.join(" ")).toMatch(/inside a compressed stream \(\/JavaScript\)/u);
  });

  it("a `stream` token inside a string literal does not swallow the object stream that follows", () => {
    const bytes = objStm("1 0 obj << /Note (stream ) >> endobj\n");
    expect(inflatedPdfStreams(bytes).unsupportedStreams + inflatedPdfStreams(bytes).opaqueStreams).toBeGreaterThanOrEqual(0);
    expect(judge(bytes).ok, "decoy string literal").toBe(false);
    expect(judge(bytes).reasons.join(" ")).toMatch(/compressed stream|not fully examined/u);
  });

  it("a `stream` token inside a comment does not swallow the object stream that follows", () => {
    expect(judge(objStm("% stream\n")).ok, "decoy comment").toBe(false);
  });

  it("a nested/escaped literal decoy does not swallow the object stream that follows", () => {
    expect(judge(objStm("1 0 obj << /Note (a \) (stream ) b) >> endobj\n")).ok, "nested decoy").toBe(false);
  });

  it("an unterminated string literal makes the file unreadable, not clean", () => {
    const body = deflateSync(L(ACTION));
    const bytes = Buffer.concat([
      L("%PDF-1.5\n1 0 obj << /Note (never closed >> endobj\n"),
      L(`2 0 obj << /Type /ObjStm /N 1 /First 0 /Filter /FlateDecode /Length ${body.byteLength} >>\nstream\n`),
      body,
      L("\nendstream\nendobj\n%%EOF\n"),
    ]);
    const result = judge(bytes);
    expect(result.ok).toBe(false);
    expect(result.reasons.join(" ")).toMatch(/not fully examined|compressed stream/u);
  });

  it("a stream start with no endstream is unreadable, not clean", () => {
    const bytes = L("%PDF-1.5\n2 0 obj << /Type /ObjStm /Filter /FlateDecode /Length 9 >>\nstream\nnot-zlib!\n%%EOF\n");
    expect(judge(bytes).reasons.join(" ")).toMatch(/not fully examined/u);
  });

  it("ordinary documents with parentheses and comments in page text are still accepted", () => {
    const content = "BT (Prices (2026) apply) Tj ET";
    const bytes = Buffer.concat([
      L("%PDF-1.4\n% generated by a normal producer\n1 0 obj << /Type /Catalog /Pages 2 0 R >> endobj\n"),
      L(`4 0 obj << /Length ${content.length} >>\nstream\n${content}\nendstream\nendobj\n`),
