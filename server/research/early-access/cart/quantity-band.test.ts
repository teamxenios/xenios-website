 ...
 *
 * One file holding the whole mandate for the one-through-fifty candidate, so a
 ...
 *      fifty in the quote, the checkout, the invoice, the settlement, the
 *      receipt, the child release and every projection. Fifty units never
 *      become fifty orders, fifty releases or fifty emails.
 */
...
describe("the band itself", () => {
  it("accepts one and fifty and refuses everything outside", () => {
    ...
    for (const accepted of [1, 2, 3, 20, 49, 50]) {
...
describe("the quote, at fifty", () => {
  ...
  it("does not let the widened band bypass Product Control", async () => {
    // The scarce unit declares a limit of 2. Fifty is inside the candidate band
  ...
  it("changes the intent identity when the quantity changes", async () => {
    ...
    expect(fifty.ok && nineteen.ok).toBe(true);
    if (!fifty.ok || !nineteen.ok) return;
    expect(fifty.quote.intentHash).not.toBe(nineteen.quote.intentHash);
  });
});
...
describe("checkout, invoice and replay at fifty", () => {
  ...
  it("refuses a checkout whose intent no longer matches the changed quantity", async () => {
    ...
    if (!fifty.ok || !nineteen.ok) throw new Error("fixture quote refused");
    const result = await checkoutEarlyAccessCart(d.checkout, CUSTOMER, {
