...
import {
  readEarlyAccessRefundHistory,
  refundedTotalCents,
} from "../commerce/refund";
import {
...
export async function earlyAccessHistoryPaymentEvidence(
  ...
  if (
    refunds === null ||
    refunds.some((refund) => refund.orderId !== placement.orderNumber) ||
    (refunds.length > 0 &&
      (captured === null || refunds.some((refund) => refund.verifiedPaidCents !== captured)))
  ) {
    return Object.freeze({ amountCapturedCents: captured, amountRefundedCents: null });
  }
  const refunded = refundedTotalCents(refunds);
  if (!Number.isSafeInteger(refunded) || refunded < 0 || (captured !== null && refunded > captured)) {
    return Object.freeze({ amountCapturedCents: captured, amountRefundedCents: null });
  }
...
export function withEarlyAccessOrderHistory(
  ...
  const available = Object.freeze({ connected: true, complete: true });
  ...
  return {
    historySources: Object.freeze({
      ...baseSources,
      xea: available,
      xec: deps.cartOrders === undefined ? unavailable : available,
    }),
    ...
    async getForMember(memberId: string, orderId: string): Promise<OrderDetailDto | null> {
      ...
      if (match !== undefined) {
        return earlyAccessOrderDetail(
          match,
          await earlyAccessHistoryPaymentEvidence(deps, match),
        );
      }
