 ...
 *    accepted first-release authority remains 20 while the candidate global
 *    band understands 50. Both single-order and cart paths must enforce it.
 *
...
import { FounderReleaseCartPricing } from "../cart/adapters";
...
describe("authority 2: accepted production authority remains twenty", () => {
  it("keeps the historical first-release seed at twenty", async () => {
  ...
  it("still refuses a release that claims a ceiling outside what a founder may approve", () => {
    ...
    // 50 is structurally approvable by the domain, but this candidate does not
    // claim that the founder has approved it. Only the separate append-only
    // release-authority chain may create that durable decision.
  ...
  it("keeps the cart at the durable release ceiling while Product Control is null", async () => {
    const unit = row();
    const ledger = new InMemoryEarlyAccessReleaseLedger();
    const appended = await ledger.append({
      releaseId: "rel-production-qty20",
      productId: unit.productId,
      variantId: unit.variantId,
      productVersion: earlyAccessReleaseVersion(unit),
      status: "approved",
      approvedPriceCents: 3_350,
      currency: "USD",
      waivedBlockers: ["QUANTITY_LIMIT_MISSING", "IMAGE_PENDING"],
      approvedQuantityLimit: 20,
      expiresAt: null,
      actor: "Samuel Boadu",
      reason: "Accepted production quantity authority remains one through twenty.",
      recordedAt: "2026-08-11T12:00:00.000Z",
    });
    expect(appended.ok).toBe(true);

    const pricing = new FounderReleaseCartPricing({
      catalog: { load: async () => ({ rows: [unit] }) } as never,
      releases: ledger,
    });
    const cartUnit = {
      productId: unit.productId,
    ...
    const common = {
      ...
      customer: { customerRef: "eac_0123456789abcdef0123456789abcdef" },
    };

    expect(await pricing.decide({ ...common, quantity: 20 })).toMatchObject({ released: true });
    expect(await pricing.decide({ ...common, quantity: 21 })).toEqual({
      released: false,
      code: "QUANTITY_INVALID",
    });
    expect(await pricing.decide({ ...common, quantity: 50 })).toEqual({
      released: false,
      code: "QUANTITY_INVALID",
    });
  });
...
describe("authority 3: a declared Product Control ceiling may express the whole band", () => {
  it("accepts a declared maxUnitsPerOrder anywhere in 1..50 and refuses 51", async () => {
    ...
    for (const declared of [EARLY_ACCESS_MIN_QUANTITY, 3, 4, 20, 49, EARLY_ACCESS_MAX_QUANTITY]) {
