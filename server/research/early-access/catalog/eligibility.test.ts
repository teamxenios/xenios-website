...
describe("assessEarlyAccessEligibility", () => {
  ...
  describe("supplier, fulfillment, quantity, documentation", () => {
    ...
    it("blocks a missing or out-of-range quantity limit", () => {
      ...
      // 51 rather than 4: the round's band is now one through fifty, so a
      // declared limit of 4 is a legal Product Control decision and only a
      // limit outside the band is a blocker.
      for (const maxUnitsPerOrder of [0, 51, 1.5, -1]) {
        expect(
          blockersFor(
            satisfied({
              variantFacts: [
    ...
    it("accepts Product Control's founder-approved maximum of fifty", () => {
      expect(
        blockersFor(
          satisfied({
            variantFacts: [
              facts({
                quantityLimit: { variantId: VARIANT_ID, maxUnitsPerOrder: 50 },
              }),
            ],
          }),
        ),
      ).not.toContain("QUANTITY_LIMIT_MISSING");
    });
