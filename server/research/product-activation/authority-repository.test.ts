...
import * as authorityRepository from "./authority-repository";
...
import { resolveActivationAuthorityFromTestRows } from "./testing/authority.test-support";

// These expected type errors are the source-contract regression: restoring
// either old production export makes the directive unused and typecheck fail.
// @ts-expect-error test-only synchronous authority repositories are not a production export
type RemovedSynchronousTestRepository = import("./authority-repository").TestOnlySynchronousProductVariantActivationLedgerRepository;
// @ts-expect-error test-only authority minting is not a production export
type RemovedSynchronousTestResolver = typeof import("./authority-repository").resolveProductVariantActivationAuthorityForTest;
void (0 as unknown as RemovedSynchronousTestRepository);
void (0 as unknown as RemovedSynchronousTestResolver);
...
describe("durable product+variant activation authority", () => {
  it("does not export a test-only authority mint from the production module", () => {
    expect(authorityRepository).not.toHaveProperty(
      "resolveProductVariantActivationAuthorityForTest",
    );
  });

  it("recomputes a canonical revision-bound fingerprint and seals only its own live result", async () => {
    const row = record();
    expect(canonicalProductVariantActivationPayload(row)).toContain('"SKU-A"');
    const evidence = await resolveActivationAuthorityFromTestRows(
      [row],
      EXACT,
    );
  ...
  it("fails closed for every non-live, stale, revoked, ambiguous, mismatched, and conflicting case", async () => {
    const cases = [
    ...
    for (const testCase of cases) {
      expect(
        (await resolveActivationAuthorityFromTestRows(
          testCase.rows,
          EXACT,
        )).state,
        testCase.label,
