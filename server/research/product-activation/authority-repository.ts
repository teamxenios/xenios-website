...
export interface TestOnlySynchronousProductVariantActivationLedgerRepository {
  /** In-memory/local equivalent used by deterministic test compositions. */
  readCurrentCandidates(input: Readonly<{
    sku: string;
    evaluatedAt: string;
...
/**
 * Deterministic local/in-memory repository resolution. This still performs a
 * repository read on every call; unlike the removed public adjudicator it does
 * not accept caller-supplied candidate rows directly. Production request paths
 * use the asynchronous durable resolver below.
 */
export function resolveProductVariantActivationAuthorityForTest(
  repository: TestOnlySynchronousProductVariantActivationLedgerRepository,
  exact: ProductVariantActivationLookup,
): ProductVariantActivationAuthorityEvidence {
  try {
    const candidates = repository.readCurrentCandidates({
      sku: exact.sku,
      evaluatedAt: exact.evaluatedAt,
    });
