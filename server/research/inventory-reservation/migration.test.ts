...
describe("atomic inventory reservation migration", () => {
  it("pins the canonical raw Git-blob checksum rather than working-tree line endings", () => {
    ...
    expect(gitBlob).toHaveLength(51_665);
  ...
  it("creates absent canonical reservation tables and converges with Track B", () => {
    ...
    expect(bootstrap).toContain(
      "By default this matches production: neither canonical",
    );
    expect(bootstrap).toContain("\\if :{?precreate_track_b_reservations}");
    expect(bootstrap).toContain("\\endif");
