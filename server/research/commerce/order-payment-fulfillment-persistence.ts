 ...
 * A final-base adapter must reconstruct this projection from canonical
 * research_orders, existing manual-payment, canonical fulfillment, and
 * canonical claim persistence, then map one `commit` call to their reviewed
 * transaction boundary. It must not create Pack 04 tables. This file performs
 * no SQL, registers no route and calls no provider.
 */
...
export interface OrderWorkflowAtomicStore {
  /** Load an orchestration projection assembled from canonical persistence. */
  load(): Promise<OrderWorkflowLoadedState>;
  /**
   * One implementation call must use the canonical transaction boundary. A
   * parallel Pack 04 order/payment/fulfillment store is not a valid adapter.
   */
...
const PROBLEM_KINDS = new Set<string>(ORDER_PROBLEM_KINDS);
const PROBLEM_STATES = new Set<string>(["reported", "claim_linked", "resolved"]);
...
function assertLoadedState(value: OrderWorkflowLoadedState): void {
  ...
  for (const order of value.snapshot.orders) {
    ...
    if (!order || typeof order.orderId !== "string" || orderIds.has(order.orderId)
      ...
      || !Array.isArray(order.tracking) || !Array.isArray(order.problems)
    ...
    for (const problem of order.problems) {
      if (!problem || typeof problem.problemRef !== "string" || problemRefs.has(problem.problemRef)
        || !PROBLEM_KINDS.has(problem.kind) || !PROBLEM_STATES.has(problem.state)
        || typeof problem.summary !== "string" || problem.summary.length < 1 || problem.summary.length > 1000
        || !Number.isFinite(Date.parse(problem.reportedAt)) || typeof problem.reportedBy !== "string"
        || (problem.canonicalClaimRef !== null && (
          typeof problem.canonicalClaimRef !== "string" || claimRefs.has(problem.canonicalClaimRef)
        ))
        || (problem.state === "reported" && problem.canonicalClaimRef !== null)
        || (problem.state === "claim_linked" && problem.canonicalClaimRef === null)
        || (problem.state === "resolved" && (
          typeof problem.resolutionRef !== "string" || !Number.isFinite(Date.parse(problem.resolvedAt ?? ""))
          || typeof problem.resolvedBy !== "string"
        ))) {
        throw new OrderWorkflowPersistenceCorruptionError();
      }
      problemRefs.add(problem.problemRef);
      if (problem.canonicalClaimRef !== null) claimRefs.add(problem.canonicalClaimRef);
    }
...
function deriveBundle(
  ...
  return Object.freeze({
    ...
    problem:
      accepted && (command.kind === "report_fulfillment_problem"
        || command.kind === "link_canonical_claim"
        || command.kind === "resolve_fulfillment_problem")
        ? (orderAfter?.problems.find((problem) => problem.problemRef === command.problemRef) ?? null)
        : null,
    fulfillmentStage:
