...
function validMutation(input: PutPersistentCartItemInput): boolean {
  ...
  return (
    ...
    input.selection.canonicalReadiness.inputVersions.length >= 1 &&
    input.selection.canonicalReadiness.domainVersions.length >= 1 &&
...
function validSelection(selection: PutPersistentCartItemInput["selection"]): boolean {
  ...
  return {
    ...
    async claimAnonymousCart(memberId: string, input: ClaimAnonymousCartInput) {
      ...
      if (
        ...
        !input.selections.every(validSelection) ||
