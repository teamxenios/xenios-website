...
describe("Pack 04 atomic persistence adapter", () => {
  ...
  it("serializes concurrent supplier and fulfillment retries without duplicate facts", async () => {
    const seededStore = new InMemoryOrderWorkflowAtomicStore();
    await throughVerification(new PersistentOrderWorkflowService(seededStore));
    const seeded = await seededStore.load();
    let arrivals = 0;
    let release!: () => void;
    const bothArrived = new Promise<void>((resolve) => { release = resolve; });
    const store = new InMemoryOrderWorkflowAtomicStore({
      initialRevision: seeded.revision,
      initialSnapshot: seeded.snapshot,
      beforeCommit: async ({ expectedRevision }) => {
        if (expectedRevision !== seeded.revision) return;
        arrivals += 1;
        if (arrivals === 2) release();
        await bothArrived;
      },
    ...
    const one = new PersistentOrderWorkflowService(store);
    const two = new PersistentOrderWorkflowService(store);
    const handoff: OrderCommand = {
      kind: "queue_supplier_handoff",
      orderId: ORDER,
      handoffRef: "handoff_pack04_concurrent",
      supplierId: SUPPLIER_ID,
      occurredAt: at(6),
    };
    const handoffResults = await Promise.all([
      one.execute(admin, key("concurrent-handoff"), handoff),
      two.execute(admin, key("concurrent-handoff"), handoff),
    ]);
    expect(handoffResults.filter((result) => result.ok && !result.replayed)).toHaveLength(1);
    expect(handoffResults.filter((result) => result.ok && result.replayed)).toHaveLength(1);

    const commands: readonly [OrderActor, string, OrderCommand][] = [
      [admin, "concurrent-release", {
        kind: "release_supplier_handoff", orderId: ORDER, occurredAt: at(7),
      }],
      [supplier, "concurrent-start", {
        kind: "start_fulfillment", orderId: ORDER, occurredAt: at(8),
      }],
      [supplier, "concurrent-tracking", {
        kind: "add_tracking", orderId: ORDER, trackingRef: "tracking_pack04_concurrent",
        carrier: "UPS", trackingNumber: "TRACK-CONCURRENT", occurredAt: at(9),
      }],
    ];
    for (const [actor, name, command] of commands) {
      const results = await Promise.all([
        one.execute(actor, key(name), command),
        two.execute(actor, key(name), command),
      ]);
      expect(results.filter((result) => result.ok && !result.replayed)).toHaveLength(1);
      expect(results.filter((result) => result.ok && result.replayed)).toHaveLength(1);
    }

    const snapshot = (await store.load()).snapshot;
    expect(snapshot.orders).toHaveLength(1);
    expect(snapshot.orders[0]?.supplierHandoff).toMatchObject({
      handoffRef: "handoff_pack04_concurrent", releasedAt: at(7),
    });
    expect(snapshot.orders[0]?.tracking).toEqual([expect.objectContaining({
      trackingRef: "tracking_pack04_concurrent",
    })]);
    expect(snapshot.orders[0]?.events.filter((event) => event.kind === "supplier_handoff_queued"))
      .toHaveLength(1);
    expect(snapshot.orders[0]?.events.filter((event) => event.kind === "supplier_handoff_released"))
      .toHaveLength(1);
    expect(snapshot.orders[0]?.events.filter((event) => event.kind === "fulfillment_started"))
      .toHaveLength(1);
    expect(snapshot.orders[0]?.events.filter((event) => event.kind === "tracking_added"))
      .toHaveLength(1);
  });
  ...
  it("refuses a changed payload racing under the same idempotency key", async () => {
    ...
    expect(results.filter((result) => "code" in result && result.code === "idempotency_conflict"))
      .toHaveLength(1);
  ...
  it("commits normalized invoice, evidence, settlement and supplier facts with their projection", async () => {
    ...
    expect((await service.execute(buyer, key("problem"), {
      kind: "report_fulfillment_problem", orderId: ORDER,
      problemRef: "problem_pack04_persist", problemKind: "damaged",
      summary: "The shipment arrived damaged.", occurredAt: at(12),
    })).ok).toBe(true);
    expect((await service.execute(admin, key("claim-link"), {
      kind: "link_canonical_claim", orderId: ORDER,
      problemRef: "problem_pack04_persist", canonicalClaimRef: "claim_pack04_persist",
      occurredAt: at(13),
    })).ok).toBe(true);
    ...
    expect(commits.find((entry) => entry.command === "report_fulfillment_problem")?.problem)
      .toMatchObject({ problemRef: "problem_pack04_persist", state: "reported" });
    expect(commits.find((entry) => entry.command === "link_canonical_claim")?.problem)
      .toMatchObject({ canonicalClaimRef: "claim_pack04_persist", state: "claim_linked" });
    expect((await service.customerTimeline(buyer, ORDER))?.stage).toBe("delivered");
    expect(await service.customerTimeline(buyer, ORDER)).toMatchObject({
      problemStatus: "claim_linked", openProblemCount: 1,
    });
