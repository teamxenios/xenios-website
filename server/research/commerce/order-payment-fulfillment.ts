...
export const ORDER_PROBLEM_KINDS = [
...
export type OrderProblemKind = (typeof ORDER_PROBLEM_KINDS)[number];
export type OrderProblemState = "reported" | "claim_linked" | "resolved";

/**
 * Projection of a problem and optional canonical-claim reference. Pack 04 does
 * not create or adjudicate claims; the existing claims service owns that work.
 */
export type OrderProblem = Readonly<{
  problemRef: string;
  kind: OrderProblemKind;
  state: OrderProblemState;
  summary: string;
  reportedAt: string;
  reportedBy: string;
  canonicalClaimRef: string | null;
...
export type OrderWorkflowEventKind =
  ...
  | "order_shipped"
  | "order_delivered"
  | "fulfillment_problem_reported"
  | "canonical_claim_linked"
  | "fulfillment_problem_resolved"
  | "order_cancelled";
...
export type OrderCommand =
  ...
  | (CommandBase & Readonly<{
      kind: "report_fulfillment_problem";
      problemRef: string;
      problemKind: OrderProblemKind;
      summary: string;
    }>)
  | (CommandBase & Readonly<{
      kind: "link_canonical_claim";
      problemRef: string;
      canonicalClaimRef: string;
    }>)
  | (CommandBase & Readonly<{
      kind: "resolve_fulfillment_problem";
      problemRef: string;
...
export type CustomerOrderTimeline = Readonly<{
  ...
  currency: "USD" | null;
  problemStatus: CustomerOrderProblemStatus;
...
export type CustomerOrderProblemStatus = "none" | "reported" | "claim_linked" | "resolved";
...
const CONSEQUENTIAL = new Set<OrderCommand["kind"]>([
  ...
  "mark_shipped",
  "mark_delivered",
  "link_canonical_claim",
  "resolve_fulfillment_problem",
  "cancel_order",
]);

const PROBLEM_KINDS = new Set<OrderProblemKind>(ORDER_PROBLEM_KINDS);
const PROBLEM_COMMANDS = new Set<OrderCommand["kind"]>([
  "report_fulfillment_problem",
  "link_canonical_claim",
  "resolve_fulfillment_problem",
]);
...
function isTerminal(stage: OrderWorkflowStage): boolean {
  return stage === "request_rejected" || stage === "cancelled";
}

function customerSafeEventDetail(
  kind: OrderWorkflowEventKind,
  detail: Readonly<Record<string, string | number | boolean | null>>,
): Readonly<Record<string, string | number | boolean | null>> {
  const allowed: Partial<Record<OrderWorkflowEventKind, readonly string[]>> = {
    buyer_request_created: ["lineCount"],
    request_rejected: ["reason"],
    invoice_issued: ["invoiceRef", "amountCents", "currency"],
    payment_verified: ["amountCents", "currency"],
    tracking_added: ["carrier", "trackingNumber"],
    fulfillment_problem_reported: ["problemRef", "problemKind"],
    canonical_claim_linked: ["problemRef", "canonicalClaimRef"],
    fulfillment_problem_resolved: ["problemRef", "resolutionRef"],
    order_cancelled: ["reason"],
  };
  return frozen(Object.fromEntries(
    (allowed[kind] ?? [])
...
function customerProblemStatus(order: OrderWorkflow): CustomerOrderProblemStatus {
  if (order.problems.some((problem) => problem.state === "claim_linked")) return "claim_linked";
  if (order.problems.some((problem) => problem.state === "reported")) return "reported";
  return order.problems.length > 0 ? "resolved" : "none";
}
...
function historyItem(order: OrderWorkflow): CustomerOrderHistoryItem {
  ...
  return frozen({
    ...
    problemStatus: customerProblemStatus(order),
    openProblemCount: order.problems.filter((problem) => problem.state !== "resolved").length,
...
export class InMemoryOrderWorkflowEngine {
  ...
  execute(actor: OrderActor, idempotencyKey: string, command: OrderCommand): OrderWorkflowResult {
    ...
    if ("code" in applied) {
      return this.refuse(actor, idempotencyKey, command, fingerprint, applied.code);
    }
  ...
  customerTimeline(actor: OrderActor, orderId: string): CustomerOrderTimeline | null {
    ...
    return frozen({
      ...
      events: order.events
        ...
        .map(({ kind, occurredAt, detail }) => ({
          kind,
          occurredAt,
          detail: customerSafeEventDetail(kind, detail),
        })),
    });
  }
  ...
  private apply(
    ...
    if (isTerminal(current.stage)
      || (current.stage === "delivered" && !PROBLEM_COMMANDS.has(command.kind))) {
    ...
    switch (command.kind) {
      ...
      case "report_fulfillment_problem": {
        const buyerMayReport = actorCanOwn(actor, current.owner)
          && (current.stage === "shipped" || current.stage === "delivered");
        const operationsMayReport = this.supplierOrAdmin(actor, current)
          && ["supplier_handoff_released", "fulfilling", "shipped", "delivered"].includes(current.stage);
        if (!buyerMayReport && !operationsMayReport) return { ok: false, code: "not_permitted" };
        if (!safeId(command.problemRef) || !PROBLEM_KINDS.has(command.problemKind)
          || !nonBlank(command.summary, 1000)) return { ok: false, code: "validation_failed" };
        if (current.problems.some((problem) => problem.problemRef === command.problemRef)) {
          return { ok: false, code: "problem_already_exists" };
        }
        const problem: OrderProblem = frozen({
          problemRef: command.problemRef,
          kind: command.problemKind,
          state: "reported",
          summary: command.summary,
          reportedAt: command.occurredAt,
          reportedBy: actor.actorId,
          canonicalClaimRef: null,
          resolutionRef: null,
          resolvedAt: null,
          resolvedBy: null,
        });
        const item = event(current, command, actor, "fulfillment_problem_reported", true, {
          problemRef: command.problemRef,
          problemKind: command.problemKind,
        });
        return { ok: true, order: nextOrder(current, command, actor, {
          problems: [...current.problems, problem],
        }, item) };
      }
      case "link_canonical_claim": {
        if (actor.role !== "admin") return { ok: false, code: "not_permitted" };
        if (!safeId(command.problemRef) || !safeId(command.canonicalClaimRef)) {
          return { ok: false, code: "validation_failed" };
        }
        const index = current.problems.findIndex((problem) => problem.problemRef === command.problemRef);
        if (index < 0) return { ok: false, code: "problem_not_found" };
        const problem = current.problems[index]!;
        if (problem.state !== "reported" || problem.canonicalClaimRef !== null
          || current.problems.some((item) => item.canonicalClaimRef === command.canonicalClaimRef)) {
          return { ok: false, code: "claim_already_linked" };
        }
        const problems = [...current.problems];
        problems[index] = frozen({
          ...problem,
          state: "claim_linked",
          canonicalClaimRef: command.canonicalClaimRef,
        });
        const item = event(current, command, actor, "canonical_claim_linked", true, {
          problemRef: command.problemRef,
          canonicalClaimRef: command.canonicalClaimRef,
        });
        return { ok: true, order: nextOrder(current, command, actor, { problems }, item) };
      }
      case "resolve_fulfillment_problem": {
        if (actor.role !== "admin") return { ok: false, code: "not_permitted" };
        if (!safeId(command.problemRef) || !safeId(command.resolutionRef)) {
          return { ok: false, code: "validation_failed" };
        }
        const index = current.problems.findIndex((problem) => problem.problemRef === command.problemRef);
        if (index < 0) return { ok: false, code: "problem_not_found" };
        const problem = current.problems[index]!;
        if (problem.state === "resolved") return { ok: false, code: "stage_invalid" };
        const problems = [...current.problems];
        problems[index] = frozen({
          ...problem,
          state: "resolved",
        ...
        const item = event(current, command, actor, "fulfillment_problem_resolved", true, {
          problemRef: command.problemRef,
          resolutionRef: command.resolutionRef,
        });
        return { ok: true, order: nextOrder(current, command, actor, { problems }, item) };
