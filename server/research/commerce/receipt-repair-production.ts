// Read-only composition for one receipt reconciliation PREVIEW. This module
// neither resolves credentials nor imports the outbox, checkout or server root.
// Queue approval is deliberately not a capability of this adapter.
...
/** No constructor or import performs a read. Each repair owns independent
 * identity bindings, so overlapping previews cannot borrow each other's rows. */
export function createReceiptRepairPreview(input: ReceiptPreviewInput) {
  ...
  async function rows(build: () => ReceiptPreviewReadQuery): Promise<unknown[]> {
  ...
  async function single(build: () => ReceiptPreviewReadQuery): Promise<unknown | null> {
    const result = await rows(build);
  ...
  return {
    async repair(options: ReceiptPreviewOptions = {}): Promise<ReceiptPreviewReport> {
      ...
      const after = position(options.cursor);
      ...
      const repair = createReceiptRepair({
        ...
        // There is no proven alias from other order-number/obligation domains.
        priorEventKeys: () => [],
        ...
        async readOrder(id): Promise<ReceiptOrderFacts | null> {
          const expected = orders.get(id);
          requireFact(expected && isUuid(id), "source_identity_mismatch");
          const value = await single(() => client.from("research_orders").select(orderKeys.join(",")).eq("id", id).limit(2));
        ...
        async recipientFor(id) {
          requireFact(members.has(id) && isUuid(id), "source_identity_mismatch");
          const value = await single(() => client.from("research_members").select("id,email").eq("id", id).limit(2));
          if (value === null) return null;
        ...
        async findQueuedEvent(key): Promise<QueuedEventFacts | null> {
          const expected = [...orders.values()].find(fact => receiptEventKey(fact.orderId) === key);
          requireFact(expected, "source_identity_mismatch");
          const value = await single(() => client.from("research_notification_outbox").select(eventKeys.join(",")).eq("event_key", key).limit(2));
          if (value === null) return null;
