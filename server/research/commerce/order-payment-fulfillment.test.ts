...
describe("Pack 04 order/payment/fulfillment workflow", () => {
  ...
  it("makes payment, fulfillment, tracking, delivery, problem and claim-link retries idempotent", () => {
    const engine = new InMemoryOrderWorkflowEngine();
    throughPaymentVerification(engine);

    const evidenceReplay = run(engine, buyer, "evidence", {
      kind: "submit_payment_evidence",
      orderId: ORDER,
      report: report(),
      occurredAt: at(4),
    });
    const verificationReplay = run(engine, finance, "verify", {
      kind: "verify_payment",
      orderId: ORDER,
      verification: verification(),
      occurredAt: at(5),
    });
    expect(evidenceReplay).toMatchObject({ ok: true, replayed: true });
    expect(verificationReplay).toMatchObject({ ok: true, replayed: true });
    expect(run(engine, finance, "second-settlement", {
      kind: "verify_payment",
      orderId: ORDER,
      verification: {
        ...verification(),
        externalTransactionRef: "external_transaction_pack04_0002",
        commitRef: "commit_pack04_0002",
      },
      occurredAt: at(5),
    })).toMatchObject({ ok: false, code: "stage_invalid" });
    expect(engine.getForActor(finance, ORDER)).toMatchObject({
      settlement: {
        settlementRef: "commit_pack04_0001",
        externalTransactionRef: "external_transaction_pack04_0001",
      },
    ...
    const steps: readonly [OrderActor, string, OrderCommand][] = [
      [admin, "retry-handoff", {
        kind: "queue_supplier_handoff", orderId: ORDER,
        handoffRef: "handoff_pack04_retry", supplierId: SUPPLIER_ID, occurredAt: at(6),
      }],
      [admin, "retry-release", {
        kind: "release_supplier_handoff", orderId: ORDER, occurredAt: at(7),
      }],
      [supplier, "retry-start", {
        kind: "start_fulfillment", orderId: ORDER, occurredAt: at(8),
      }],
      [supplier, "retry-tracking", {
        kind: "add_tracking", orderId: ORDER, trackingRef: "tracking_pack04_retry",
        carrier: "UPS", trackingNumber: "1Z999AA10123456784", occurredAt: at(9),
      }],
      [supplier, "retry-shipped", {
        kind: "mark_shipped", orderId: ORDER, occurredAt: at(10),
      }],
      [supplier, "retry-delivered", {
        kind: "mark_delivered", orderId: ORDER, occurredAt: at(11),
      }],
      [buyer, "retry-problem", {
        kind: "report_fulfillment_problem", orderId: ORDER,
        problemRef: "problem_pack04_0001", problemKind: "damaged",
        summary: "Package contents were damaged in transit.", occurredAt: at(12),
      }],
      [admin, "retry-claim-link", {
        kind: "link_canonical_claim", orderId: ORDER,
        problemRef: "problem_pack04_0001", canonicalClaimRef: "claim_pack04_0001", occurredAt: at(13),
      }],
      [admin, "retry-problem-resolution", {
        kind: "resolve_fulfillment_problem", orderId: ORDER,
        problemRef: "problem_pack04_0001", resolutionRef: "resolution_pack04_0001", occurredAt: at(14),
      }],
    ];

    for (const [actor, key, command] of steps) {
      const accepted = run(engine, actor, key, command);
      expect(accepted).toMatchObject({ ok: true, replayed: false });
      const acceptedVersion = accepted.ok ? accepted.order.version : -1;
      expect(run(engine, actor, key, command)).toMatchObject({
        ok: true, replayed: true, order: { version: acceptedVersion },
      });
    }

    const order = engine.getForActor(buyer, ORDER);
    expect(order).toMatchObject({
      stage: "delivered",
      settlement: { settlementRef: "commit_pack04_0001" },
      supplierHandoff: { handoffRef: "handoff_pack04_retry" },
      tracking: [{ trackingRef: "tracking_pack04_retry" }],
      problems: [{
        problemRef: "problem_pack04_0001",
        state: "resolved",
        canonicalClaimRef: "claim_pack04_0001",
        resolutionRef: "resolution_pack04_0001",
      }],
    });
    expect(order?.paymentEvidence).toHaveLength(1);
    expect(order?.tracking).toHaveLength(1);
    expect(order?.problems).toHaveLength(1);

    const timeline = engine.customerTimeline(buyer, ORDER);
    expect(timeline).toMatchObject({ problemStatus: "resolved", openProblemCount: 0 });
    expect(engine.customerOrderHistory(buyer).orders[0]).toMatchObject({
      orderId: ORDER, problemStatus: "resolved", openProblemCount: 0,
    });
    expect(timeline?.events.slice(-3).map((item) => item.kind)).toEqual([
      "fulfillment_problem_reported", "canonical_claim_linked", "fulfillment_problem_resolved",
    ]);
    expect(JSON.stringify(timeline)).not.toContain("Package contents were damaged");
    expect(engine.audits().filter((entry) => entry.outcome === "replayed")).toHaveLength(11);
  });

  it("allowlists customer event detail even when a canonical adapter supplies hostile extra keys", () => {
    const engine = new InMemoryOrderWorkflowEngine();
    expect(run(engine, buyer, "hostile-detail-source", request())).toMatchObject({ ok: true });
    const snapshot = engine.snapshot();
    const order = snapshot.orders[0]!;
    const firstEvent = order.events[0]!;
    const rehydrated = new InMemoryOrderWorkflowEngine({
      ...snapshot,
      orders: [{
        ...order,
        events: [{
          ...
          detail: {
            ...firstEvent.detail,
            privateObjectRef: "private/manual-payment-proofs/secret.png",
            externalTransactionRef: "external_transaction_secret",
            supplierId: "supplier_secret",
            internalAuditNote: "never expose this",
          },
        }],
      }],
    });
    const timeline = rehydrated.customerTimeline(buyer, ORDER);
    expect(timeline?.events).toEqual([{
      kind: "buyer_request_created",
      occurredAt: at(0),
      detail: { lineCount: 1 },
    }]);
    const serialized = JSON.stringify(timeline);
    expect(serialized).not.toContain("private/manual-payment-proofs");
    expect(serialized).not.toContain("external_transaction_secret");
    expect(serialized).not.toContain("supplier_secret");
    expect(serialized).not.toContain("never expose this");
  });

  it("refuses duplicate order, fulfillment, tracking, claim and out-of-order creation attempts", () => {
    const engine = new InMemoryOrderWorkflowEngine();
    expect(run(engine, buyer, "duplicate-order-first", request())).toMatchObject({ ok: true });
    expect(run(engine, buyer, "duplicate-order-second", request())).toMatchObject({
      ok: false, code: "order_already_exists",
    });
    expect(run(engine, admin, "approve", {
    ...
    expect(run(engine, admin, "handoff", {
      kind: "queue_supplier_handoff", orderId: ORDER,
      handoffRef: "handoff_pack04_0001", supplierId: SUPPLIER_ID, occurredAt: at(6),
    })).toMatchObject({ ok: true });
    expect(run(engine, admin, "second-handoff", {
      kind: "queue_supplier_handoff", orderId: ORDER,
      handoffRef: "handoff_pack04_0002", supplierId: SUPPLIER_ID, occurredAt: at(7),
    })).toMatchObject({ ok: false, code: "stage_invalid" });
    expect(run(engine, admin, "release", {
      kind: "release_supplier_handoff", orderId: ORDER, occurredAt: at(7),
    })).toMatchObject({ ok: true });
    expect(run(engine, supplier, "start", {
      kind: "start_fulfillment", orderId: ORDER, occurredAt: at(8),
    })).toMatchObject({ ok: true });
    expect(run(engine, supplier, "second-start", {
      kind: "start_fulfillment", orderId: ORDER, occurredAt: at(9),
    })).toMatchObject({ ok: false, code: "stage_invalid" });
    expect(run(engine, supplier, "tracking", {
      kind: "add_tracking", orderId: ORDER, trackingRef: "tracking_pack04_0001",
      carrier: "UPS", trackingNumber: "TRACK-1", occurredAt: at(9),
    })).toMatchObject({ ok: true });
    expect(run(engine, supplier, "tracking-duplicate-ref", {
      kind: "add_tracking", orderId: ORDER, trackingRef: "tracking_pack04_0001",
      carrier: "UPS", trackingNumber: "TRACK-2", occurredAt: at(10),
    })).toMatchObject({ ok: false, code: "idempotency_conflict" });
    expect(run(engine, supplier, "stale-time", {
      kind: "mark_shipped", orderId: ORDER, occurredAt: at(8),
