...
const profile: BusinessProfile = {
  ...
  billingEmail: "k@romandigital.io",
  phone: null,
  taxIdLast4: null,
...
function dashboard(): Omit<OrganizationDashboardDto, "organization"> {
  return { profile, users: [], orders: [], openRequestAgainCount: 0 };
}
...
function deps(): AccountIdentityDeps {
  ...
  return {
    ...
    findCustomerByRef: vi.fn(async () => ({ customerRef: CUSTOMER_REF, normalizedEmail: "k@romandigital.io" })),
...
describe("Pack 02 account identity service", () => {
  ...
  it("requires Supabase-confirmed email before returning any account context", async () => {
    vi.mocked(subject.resolveAuthenticatedUser).mockResolvedValue({ userId: "u", email: "k@romandigital.io", emailVerified: false });
  ...
  it("projects personal and organization access from one Supabase identity", async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      value: {
        auth: { email: "k@romandigital.io", emailVerified: true },
