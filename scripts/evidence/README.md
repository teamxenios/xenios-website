...
| `ROUTE_LOCATION` / `ROUTE_STATE_CONTRACT`                                                  | The browser is on the declared inventory path (or `expectedBrowserPath`) and that route's required/forbidden selectors and text prove the expected state. When `expectedBrowserReturnTo` is declared, the raw query must equal the single encoded `returnTo` parameter exactly; missing, wrong, duplicate, or extra parameters fail. Missing semantic contracts or not-applicable state evidence are invalid for required routes. |
