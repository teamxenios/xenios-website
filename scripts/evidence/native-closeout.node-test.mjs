...
test("real native workflows through local auth, with ownership and failure boundaries", async () => {
  ...
  try {
    ...
    const memberOrders = (await call("/api/research/orders", member)).body;
    assert.equal(memberOrders.requests.length, 2);
    assert.equal(memberOrders.requests[0].publicReference, FIXTURE.requestReference);
    assert.equal(memberOrders.requests[0].estimatedTotalCents, null);
    assert.equal(memberOrders.requests[1].trackingReference, "SYNTHETIC-OPAQUE-TRACKING");
    assert.deepEqual(memberOrders.requestsSource, { connected: true, complete: true });
    assert.ok(!JSON.stringify(memberOrders.requests).includes("actorMemberId"));
    assert.deepEqual((await call("/api/research/orders", other)).body.requests, []);
    const accountOrders = (await call("/api/research/customer-account/orders", member)).body.data;
    assert.deepEqual(accountOrders.requests, memberOrders.requests);
    assert.deepEqual(accountOrders.history.sources.xrr, memberOrders.requestsSource);
    assert.deepEqual((await call("/api/research/customer-account/orders", other)).body.data.requests, []);
    assert.equal((await call("/api/research/customer-account/orders")).status, 401);
    await call("/__native_preview/requests-source", admin, "POST", { available: false });
    const incompleteOrders = (await call("/api/research/orders", member)).body;
    assert.equal(incompleteOrders.orders.length, 1);
    assert.deepEqual(incompleteOrders.requests, []);
    assert.deepEqual(incompleteOrders.requestsSource, { connected: false, complete: false });
    assert.equal((await call("/api/research/customer-account/orders", member)).body.data.history.availability, "partial");
    await call("/__native_preview/requests-source", admin, "POST", { available: true });
