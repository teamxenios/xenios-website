...
export default function HeroSection() {
  return (
    <section className="relative pt-32 pb-12 lg:pt-48 lg:pb-32 px-6 border-b border-border min-h-[90vh] flex items-center">
      ...
      <div className="container mx-auto grid lg:grid-cols-2 gap-16 items-center">
        <div className="flex flex-col gap-8">
          ...
          >
             <button onClick={() => document.getElementById('waitlist')?.scrollIntoView({behavior: 'smooth'})} className="bg-primary text-primary-foreground h-14 px-8 rounded-full text-lg font-medium hover:scale-105 transition-transform">
               Start your journey
             </button>
          </motion.div>
        </div>
...
function FeatureTile({ icon: Icon, title, text, delay }: { icon: any, title: string, text: string, delay: number }) {
  return (
    <motion.div 
      ...
      className="group p-6 border border-border bg-background hover:bg-secondary/20 transition-colors flex gap-6 items-start rounded-xl"
    >
