  ...
  return (
    <article className="xenios-order-card" data-testid={`order-card-${item.variantId}`}>
      <div className="xenios-order-card__header">
        <div>
          <p className="xenios-order-eyebrow">{familyLabel(item.family)}</p>
        ...
        <section className="xenios-order-catalog-layout">
          <div>
            <div className="xenios-order-panel xenios-order-filters">
              ...
              <label>Family<select value={family} onChange={(e) => { setFamily(e.target.value); setPage(1); }}><option value="">All families</option>{catalog?.families.map((value) => <option key={value} value={value}>{familyLabel(value)}</option>)}</select></label>
