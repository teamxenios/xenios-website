import type { CareStatusDto, CareTimelineStage } from "@shared/research/customer-account/contract";
...
export function AccountCareView({ data }: { data: CareStatusDto }) {
  if (!data.stage) {
    return (
      <section className="account-surface" aria-labelledby="care-not-started-heading">
        ...
        <h2 id="care-not-started-heading" className="account-section-title">No Care enrollment is attached.</h2>
        <p className="body-s text-ink-2 mt-3">Membership and Research access can exist without a Care relationship.</p>
      </section>
    );
  }

  const currentIndex = CARE_TIMELINE_STAGES.indexOf(data.stage);
  return (
    <div className="account-grid account-grid-main">
      <section className="account-surface" aria-labelledby="care-current-heading">
        ...
        <h2 id="care-current-heading" className="account-section-title">{STAGE_LABELS[data.stage]}</h2>
        <p className="body-m text-ink-2 mt-4 max-w-[60ch]">{data.neutralSummary ?? "Your current operational status is recorded in the Care workflow."}</p>
        <p className="mono-label text-ink-mute mt-4">Updated {formatAccountDate(data.updatedAt, true)}</p>
