 ...
 *
 * This file pins the BEHAVIOUR of the public boundary that PR #160 activated
 * (isPublicResearchPath inside layout.tsx): with the gate genuinely locked,
 * does the layout render the page or the password wall? Nothing else on main
 * tests the client wall behaviourally per route, so this is the guard that
 * catches a silent rewiring of the layout gate.
 *
...
describe("public entry, with the shared review gate LOCKED", () => {
  const OPEN = [
    ...
    // PR #160 deliberately made /research/policies/ a PUBLIC PREFIX (all policy
    // documents are served by the open GET /api/research/policies anyway), so
    // every policy slug renders publicly. Pinned so a narrowing is a conscious
    // decision rather than a drift.
    ["/research/policies/research-use", "public via the #160 policies prefix"],
  ] as const;
