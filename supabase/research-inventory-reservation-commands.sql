...
-- movement commands and converges with the dormant Track B reservation schema
-- when it exists. Production did not deploy Track B, so the two canonical base
-- reservation tables and indexes are created when absent before their accepted
-- command hardening is applied. No parallel reservation model is introduced.
-- Checkout remains disabled until a later integration adopts the port.
