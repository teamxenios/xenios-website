...
--
-- Additive and idempotent (apply-twice safe). It creates one table, three
-- functions, and one trigger. It drops no table, no column, and no row, and it
-- rewrites no price, variant, or product data. Apply after migration
-- 20260726143000. It is NOT applied by the change that introduces it.

-- Preconditions. A safety gate must never be a silent no-op, so an absent target
-- is an error rather than a notice: if this raises, apply 20260726143000 first.
do $precondition$
begin
  if to_regclass('public.research_product_prices') is null
    ...
    raise exception
      ...
      'public.research_product_variants must exist; apply migration '
      '20260726143000_research_product_control_center.sql first.';
  end if;
end
$precondition$;
