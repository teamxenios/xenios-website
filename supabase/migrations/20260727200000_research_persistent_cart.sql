...
-- This is the exact SHA-256 fingerprint emitted by the live member-catalog
-- inventory projection. Keep the property order and compact JSON encoding in
-- lockstep with inventoryFactsForVariant() in member-catalog-service.ts.
create or replace function public.research_persistent_cart_inventory_source_version(
  p_product_id uuid,
  p_variant_id uuid,
  p_sku text,
...
as $$
  select encode(public.digest(convert_to(
    coalesce((
      select '[' || string_agg(
        format(
          '{"id":%s,"productId":%s,"variantId":%s,"sku":%s,"disposition":%s,"version":%s,"updatedAt":%s,"allocatable":%s}',
          to_json(l.id::text)::text,
          to_json(l.product_id::text)::text,
          to_json(l.variant_id::text)::text,
          to_json(l.sku)::text,
          to_json(l.disposition)::text,
          l.version::text,
          to_json(l.updated_at)::text,
          case
            when public.research_lot_is_allocatable(l.id, p_evaluated_at)
              then 'true'
            else 'false'
          end
        ),
...
begin
  if p_owner_kind not in ('member','anonymous')
     ...
     or p_selection->'inventoryEligibility'->>'sourceVersion'
        !~ '^[a-f0-9]{64}$'
     or p_selection->'inventoryEligibility'->>'evaluatedAt'
        <> p_selection->>'evaluatedAt'
  ...
  if not exists (
    ...
    join public.research_product_media m on m.product_id = p.id
    where p.id = (p_selection->>'productId')::uuid
      ...
      and m.id=(p_selection->'media'->>'id')::uuid
      and m.kind='primary_image' and m.state='approved'
      and m.approved_by is not null and m.approved_at is not null
      and m.alt_text=p_selection->'media'->>'altText'
    for update of p,v,r,m
  ) then return false; end if;
...
begin
  ...
  if not public.research_persistent_cart_selection_current(
    p_owner_kind,p_owner_identity,p_selection
  ) then
...
revoke all on function public.research_persistent_cart_inventory_source_version(uuid,uuid,text,timestamptz) from public, anon, authenticated, service_role;
revoke all on function public.research_persistent_cart_selection_current(text,text,jsonb) from public, anon, authenticated, service_role;
