...
from information_schema.columns where table_schema='public' and table_name in
 ('research_members','research_partners','research_partner_links','research_attribution_touches','research_idempotency_keys',
  'research_affiliate_customer_bindings','research_referral_binding_transfer_events')
order by table_name,ordinal_position;
select c.relname,k.conname,k.contype,k.convalidated,pg_get_constraintdef(k.oid) as definition
from pg_constraint k join pg_class c on c.oid=k.conrelid join pg_namespace n on n.oid=c.relnamespace
where n.nspname='public' and c.relname in ('research_members','research_partners','research_partner_links','research_attribution_touches',
  'research_idempotency_keys','research_affiliate_customer_bindings','research_referral_binding_transfer_events')
order by c.relname,k.conname;
select tablename,indexname,indexdef from pg_indexes where schemaname='public' and tablename in
 ('research_members','research_partners','research_partner_links','research_attribution_touches','research_idempotency_keys',
  'research_affiliate_customer_bindings','research_referral_binding_transfer_events') order by tablename,indexname;
