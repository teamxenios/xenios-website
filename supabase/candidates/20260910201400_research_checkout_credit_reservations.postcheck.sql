...
begin
  ...
  if not has_table_privilege('service_role','public.research_checkout_credit_reservations','SELECT')
    or has_table_privilege('service_role','public.research_checkout_credit_reservations','INSERT,UPDATE,DELETE,TRUNCATE') then
  ...
  foreach v_signature in array array[
    ...
    v_oid:=to_regprocedure(v_signature);
    if v_oid is null or not exists(select 1 from pg_catalog.pg_proc where oid=v_oid and prosecdef
        and 'search_path=""'=any(proconfig)) then raise exception 'credit_commit_definer_contract_missing'; end if;
    if not has_function_privilege('service_role',v_oid,'EXECUTE')
       or has_function_privilege('anon',v_oid,'EXECUTE') or has_function_privilege('authenticated',v_oid,'EXECUTE') then
      raise exception 'credit_commit_execute_acl_mismatch';
    end if;
  end loop;
